...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    // checks if a prompt exists in the form state
    if (form.prompt) {
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.name && form.prompt && form.photo) {
      ...
      alert(
        "Please enter your name, the prompt, and the photo you want to save"
      );
    }
  };
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => {
      return { ...prev, [name]: value }; // the square brackets are used to dynamically set a property name on the new object
    });
  };
  ...
  const handleSurpriseMe = () => {
    ...
    setForm((prev) => {
      return { ...prev, prompt: randomPrompt };
    });
  };
